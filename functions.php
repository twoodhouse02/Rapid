...
function multiblock_register_blocks()
{
    ...
    register_block_type(__DIR__ . "/build/blocks/navigation-menu");
    register_block_type(__DIR__ . "/build/blocks/navigation-link");
...
// TELEMETRY TESTING - Direct to Supabase
...
add_action("save_post", function ($post_id) {
    ...
    foreach ($post_ids as $current_post_id) {
        ...
        $post_block_counts = count_blocks_recursive($blocks, $namespace);

        // Add this post's counts to the total
        foreach ($post_block_counts as $block_name => $count) {
            if (isset($block_counts[$block_name])) {
                $block_counts[$block_name] += $count;
            } else {
                $block_counts[$block_name] = $count;
    ...
    // Only send when there are blocks found (optional)
    if (array_sum($block_counts) === 0) {
        return;
    }

    $payload = [
        "client_url" => home_url(),
        "blocks" => $block_counts, // maps to JSONB column
    ];
    ...
    if (!defined("SUPABASE_SERVICE_ROLE_KEY")) {
        error_log("ERROR: SUPABASE_SERVICE_ROLE_KEY not defined!");
        return;
    }
    ...
    if (is_wp_error($response)) {
        ...
        error_log("Supabase Response: HTTP {$code} — {$body}");
    }
});

add_action("http_api_debug", "telemetry_http_debug", 10, 5);
function telemetry_http_debug($response, $context, $class, $args, $url)
{
    if (strpos($url, "supabase.co/rest/v1/") === false) {
        return; // Only log relevant calls
    }
    error_log("　--- HTTP API Debug ---"); // prefix with a space to keep log tidy
    error_log("URL: " . $url);
